...
func (ack *SetInventoryContents) Run() {
	...
	for index, itemInstance := range ack.contents {
		if itemInstance.Stack.NetworkID == 0 {
			...
			inv.SetSlot(index, utils.ReadItem(itemInstance.Stack.NBTData, new(ack.mPlayer.ConvertToStack(itemInstance.Stack))))
		}
	}
}
...
func (ack *SetInventorySlot) Run() {
	...
	if ack.item.Stack.NetworkID == 0 {
		...
		inv.SetSlot(int(ack.slot), utils.ReadItem(ack.item.Stack.NBTData, new(ack.mPlayer.ConvertToStack(ack.item.Stack))))
	}
}
